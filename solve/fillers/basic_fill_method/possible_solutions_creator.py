...
class PossibleSolutionsCreator:
    ...
    @staticmethod
    def possible_solutions_creator(board: array) -> List[Dict[int, set]]:
        ...
        for i in range(9):
            hash_map_row[i] = PossibleValuesChecker.check_column_row(board[i, :])
            hash_map_column[i] = PossibleValuesChecker.check_column_row(board[:, i])
